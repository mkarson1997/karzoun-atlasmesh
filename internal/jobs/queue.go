...
func NewWithClock(now func() time.Time) *Queue {
	...
	return &Queue{jobs: make(map[string]Job), now: now}
}
...
func (q *Queue) Enqueue(job Job) (Job, error) {
	...
	if job.ID == "" || job.Type == "" {
		return Job{}, errors.New("job id and type are required")
	}
	if job.MaxAttempts == 0 {
		job.MaxAttempts = 3
	}
	if job.MaxAttempts < 1 || job.MaxAttempts > 100 {
		return Job{}, errors.New("max_attempts must be between 1 and 100")
	}
	if len(job.Payload) > 1<<20 {
		return Job{}, errors.New("payload exceeds 1 MiB")
	}
...
func (q *Queue) Claim(worker string, lease time.Duration) (Job, error) {
	...
	if worker == "" {
		return Job{}, errors.New("worker is required")
	}
	if lease < time.Second || lease > time.Hour {
		return Job{}, errors.New("lease must be between 1s and 1h")
	}
	now := q.now().UTC()
	q.mu.Lock()
	defer q.mu.Unlock()
	...
	for _, id := range q.order {
		job := q.jobs[id]
		if job.State != Queued || job.AvailableAt.After(now) {
			continue
		}
		job.State = Leased
		job.Attempt++
...
func (q *Queue) Heartbeat(id, worker string, lease time.Duration) (Job, error) {
	...
	q.mu.Lock()
	defer q.mu.Unlock()
	job, err := q.assertLeaseLocked(id, worker, now)
	if err != nil {
		return Job{}, err
	}
	job.LeaseUntil = now.Add(lease)
...
func (q *Queue) Complete(id, worker string, result json.RawMessage) (Job, error) {
	if len(result) > 1<<20 {
		return Job{}, errors.New("result exceeds 1 MiB")
	}
	if len(result) != 0 && !json.Valid(result) {
		return Job{}, errors.New("result must contain valid JSON")
	}
	now := q.now().UTC()
	q.mu.Lock()
	defer q.mu.Unlock()
...
func (q *Queue) Fail(id, worker, reason string, retryAfter time.Duration) (Job, error) {
	if retryAfter < 0 || retryAfter > 24*time.Hour {
		return Job{}, errors.New("retry_after must be between 0 and 24h")
	}
	reason = strings.TrimSpace(reason)
	if len(reason) > 2048 {
		return Job{}, errors.New("error reason exceeds 2048 bytes")
	}
	now := q.now().UTC()
	q.mu.Lock()
	defer q.mu.Unlock()
	...
	if job.Attempt >= job.MaxAttempts {
		...
		job.State = Queued
		job.AvailableAt = now.Add(retryAfter)
	}
	q.jobs[id] = job
	return cloneJob(job), nil
}
...
func (q *Queue) assertLeaseLocked(id, worker string, now time.Time) (Job, error) {
	...
	if job.State != Leased || job.LeaseOwner != worker || !job.LeaseUntil.After(now) {
...
func (q *Queue) reclaimExpiredLocked(now time.Time) {
	for id, job := range q.jobs {
		if job.State != Leased || job.LeaseUntil.After(now) {
			continue
		}
		job.LeaseOwner = ""
		job.LeaseUntil = time.Time{}
...
func (s State) Valid() bool {
	switch s {
	case "", Queued, Leased, Completed, DeadLetter:
		return true
	default:
		return false
	}
...
func (j Job) String() string {
	return fmt.Sprintf("job{id=%s type=%s state=%s attempt=%d/%d}", j.ID, j.Type, j.State, j.Attempt, j.MaxAttempts)
}

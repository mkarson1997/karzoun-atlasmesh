...
func (r *Registry) Register(input RegisterInput) (Instance, error) {
	...
	if input.Service == "" || input.ID == "" || input.Address == "" { return Instance{}, errors.New("service, id and address are required") }
	if input.Weight == 0 { input.Weight = 1 }
	if input.Weight < 1 || input.Weight > 1000 { return Instance{}, errors.New("weight must be between 1 and 1000") }
	if err := validateTTL(input.TTL); err != nil { return Instance{}, err }
	...
	defer r.mu.Unlock()
	r.reapLocked(now)
	if _, ok := r.instances[input.Service]; !ok { r.instances[input.Service] = make(map[string]Instance) }
...
func (r *Registry) Heartbeat(service, id string, ttl time.Duration) (Instance, error) {
	if err := validateTTL(ttl); err != nil { return Instance{}, err }
	now := r.now().UTC()
	r.mu.Lock()
	defer r.mu.Unlock()
	r.reapLocked(now)
	instance, ok := r.instances[service][id]
	if !ok { return Instance{}, ErrNotFound }
...
func (r *Registry) Resolve(service string) (Instance, error) {
	...
	for _, instance := range r.instances[service] {
		if instance.Healthy { candidates = append(candidates, instance); totalWeight += instance.Weight }
	}
	if len(candidates) == 0 || totalWeight == 0 { return Instance{}, ErrNoHealthyNode }
	...
	for _, instance := range candidates {
		...
		if cursor < cumulative { return cloneInstance(instance), nil }
	}
...
func (r *Registry) List(service string) []Instance {
	...
	if service != "" {
		for _, instance := range r.instances[service] { result = append(result, cloneInstance(instance)) }
	} else {
		for _, byID := range r.instances { for _, instance := range byID { result = append(result, cloneInstance(instance)) } }
	}
	sort.Slice(result, func(i, j int) bool { if result[i].Service == result[j].Service { return result[i].ID < result[j].ID }; return result[i].Service < result[j].Service })
	return result
}

func (r *Registry) ReapExpired() int { r.mu.Lock(); defer r.mu.Unlock(); return r.reapLocked(r.now().UTC()) }

func (r *Registry) reapLocked(now time.Time) int {
	removed := 0
	for service, byID := range r.instances {
		for id, instance := range byID { if !instance.LeaseExpiresAt.After(now) { delete(byID, id); removed++ } }
		if len(byID) == 0 { delete(r.instances, service); delete(r.cursors, service) }
	}
	return removed
}

func validateTTL(ttl time.Duration) error {
	if ttl < minTTL || ttl > maxTTL { return fmt.Errorf("ttl must be between %s and %s", minTTL, maxTTL) }
	return nil
}
...
func cloneMetadata(input map[string]string) map[string]string {
	...
	for k, v := range input { cloned[k] = v }
	return cloned
}

func cloneInstance(instance Instance) Instance { instance.Metadata = cloneMetadata(instance.Metadata); return instance }

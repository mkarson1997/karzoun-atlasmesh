...
func NewWithClock(now func() time.Time) *Membership {
	if now == nil {
		now = time.Now
	}
	return &Membership{nodes: make(map[string]Node), now: now}
}
...
func (m *Membership) Register(id, address, zone string, ttl time.Duration) (Node, error) {
	...
	if id == "" || address == "" {
		return Node{}, errors.New("node id and address are required")
	}
	if ttl < time.Second || ttl > time.Hour {
		return Node{}, errors.New("ttl must be between 1s and 1h")
	}
	now := m.now().UTC()
	node := Node{ID: id, Address: address, Zone: zone, StartedAt: now, LastHeartbeat: now, ExpiresAt: now.Add(ttl)}
	m.mu.Lock()
	defer m.mu.Unlock()
	m.reapLocked(now)
...
func (m *Membership) ReapExpired() int {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.reapLocked(m.now().UTC())
}
func (m *Membership) reapLocked(now time.Time) int {
	removed := 0
